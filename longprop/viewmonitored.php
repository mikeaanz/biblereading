...
<div class="col-xl-12 col-lg-7">
  <div class="card shadow mb-1">
    <h6 class="m-3 font-weight-bold text-primary">MONITORED CONTACT DURING <?php echo $rows['MONTH'];?> <?php echo $rows['YR'];?> <?php echo $rows['BATCH'];?> <?php echo $rows['week'];?> <?php echo $rows['LEVEL'];?></h6>
    
    <div class="card-body">
    
    <a href="#"  onclick="window.open(
    'pdfweekform.php?SessionId=<?php echo $session_id?>&month=<?php echo $rows['MONTH'];?>&year=<?php echo $rows['YR'];?>&TermNumber=<?php echo $rows['BATCH'];?>&level=<?php echo $rows['LEVEL'];?>', 'newwindow', 'width=900, height=600'); return false;" class="btn btn-light  btn-smll"><i class="fas fa-download fa-sm text-dark-50"></i> PDF</a>   
    ...
    <a  href="#addme" class="btn btn-info  btn-smll"  data-toggle="modal" data-target="#ariveme"><i class="fa fa-train" data-toggle="tooltip" title="Upon Arrival"></i> </a>
   ...
   <?php include  'functions/modaldeletecontact.php'?>
                   <?php include 'functions/afterarrmoda.php' ?>
  ...
  <div class="card-body">
    ...
    <table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">	
                  ...
                  <?php
                           ...
                           where longproprpt.acc_id='$session_id'  or contatcdetails.contact_id='$get_id'

                          ORDER BY longproprpt_id ASC")or die(mysql_error());
													while($row = mysql_fetch_array($user_query)){

                           $wwk = $row['longproprpt_id'];
                          // $loc = $row['LOCALITY'];
                          // $user = $row['USER_LEVEL'];
													?>
                        <!--END OF CODE ---->
                    ...
                    <td width="40px"><?php echo $row['address'];?></td>
                      <td width="40px"><?php echo $row['ContactNumber'];?></td>
                             <td width="40px"><?php echo $row['Sun_LD'];?></td>
                      <td width="40px"><?php echo $row['Week_ONE'];?></td>
                      <td width="40px"><?php echo $row['Week_TWO'];?></td>
                      <td width="40px"><?php echo $row['Week_THREE'];?></td>
                      <td width="40px"><?php echo $row['Week_FOUR'];?></td>
                      <td width="40px"><?php echo $row['Week_FIVE'];?></td>
