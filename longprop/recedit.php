...
<div class="col-xl-12">
  <div class="card shadow mb-4">
    ...
    <label class="control-label" >Name of Brother and Sister:</label>
			...
			<input class="form-control form-control-lg" onkeypress="return isNumberKey(event)" style="background-color:#aed6f1;" data-toggle="datepicker" readonly  id="usr1" name="daterec" value="<?php echo $row['Date_recovered'];?>"   placeholder="mm-dd-yyyy" autocomplete="off" required/>

			<input class="form-control form-control-lg dates" name="timesup" id="price" type="hidden" value="<?php echo date('m/d/Y');  ?>" />

  
            <label class="control-label">Cell-Phone #:</label>
			<input class="form-control form-control" name="cellss" id="price" type="text" placeholder="Example +6393045311711" value="<?php echo $row['cellphone'];?>"   onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" maxlength="12"  required/>
...
if($activated > 0){
    ...
    `Name`='$name',`Home_address`='$homeadd',`Status`='$userlevel',`cellphone`='$cell',`Date_recovered`='$daterec' Where rec_id='$ids' ")or die(mysql_error());
	echo '<script> swal({title: "Praise the Lord!",text: "Your Data Successfully Updated!", customClass: "swal-wide",
        confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("editrecovered.php?historyfeedbackid='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
      exit();
...
echo '<script> swal({title: "Notice!",text: "Your data already existing.", customClass: "swal-wide",
    confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("editrecovered.php?historyfeedbackid='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
  exit();
