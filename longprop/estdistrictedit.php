  ...
  <div class="card shadow mb-4">
    ...
    <?php
     ...
     ?>
			...
			<input value="<?php echo $row['daterecovered'];?>" class="form-control form-control-lg" data-toggle="datepicker" readonly onkeypress="return isNumberKey(event)" style="background-color:#aed6f1;"  id="usr1" name="date"  placeholder="mm-dd-yyyy" autocomplete="off" required/>
...
if($activated > 0){
   ...
   `districtname`='$name',
	...
	echo '<script> swal({title: "Praise the Lord!",text: "Your Data Successfully Updated!", customClass: "swal-wide",
        confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("editrecdistrict.php?historyfeedbackid='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
      exit();
...
echo '<script> swal({title: "Notice!",text: "Your data already existing.", customClass: "swal-wide",
    confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("editrecdistrict.php?historyfeedbackid='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
  exit();
