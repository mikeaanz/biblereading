...
if (isset($_POST['delete_user'])){
    ...
    echo '<script> swal({title: "Warning!",text: "Please Check the checkbox before to proceed!", customClass: "swal-wide",
      confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("prospecttrain.php?locate_idpost='.$historyfeedbackid.'&BATCH='.$getbatch.'","_self")});</script>';
    exit();
}else
$id=$_POST['selector'];
$N = count($id);
...
echo '<script> swal({title: "Amen!",text: "This record is Successfully Deleted!", customClass: "swal-wide",
	confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("prospecttrain.php?locate_idpost='.$historyfeedbackid.'&BATCH='.$getbatch.'","_self")});</script>';
  exit();
