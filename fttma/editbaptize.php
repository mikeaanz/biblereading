...
<div class="row">
  ...
  <div class="col-lg-12">
					  ...
					  <?php
												  $user_query = mysql_query("SELECT * FROM `baptism_rpt`  where baptism_id='$contacid'
												  ")or die(mysql_error());
												  while($row = mysql_fetch_array($user_query)){
						  $id=$row['baptism_id'];
                          $contactstatus=$row['baptism_id'];
												  ?>
					  <!--END OF CODE ---->
	...
	<div class="card shadow mb-4">
	  <div class="card-header py-3">
		<h6 class="m-0 font-weight-bold text-primary text-center">Update Baptize Information</h6>
	  </div>
	  <div class="card-body">
  ...
  <div class="form-group">
    ...
    <input type="text" name="bap" class="form-control" data-toggle="datepicker" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Ex.12/12/2020">
  </div> 

  <div class="form-group">
			<label class="control-label" >Status Saints Gender :</label>
                  <Select class="browser-default custom-select" name="genders" id="userlevel" required>

                     <option  disabled>Select Saints Gender</option>
                              <?php
                                  $query = mysql_query("select * from stat_saints where stat_id='$genderid'")or die(mysql_error());
...
/* check activated */
$act = "SELECT * FROM baptism_rpt WHERE  baptism_id='$get_id'";
...
if($activated > 0){
    mysql_query("UPDATE `baptism_rpt` SET `FullName`='$fullname',`ContactNumber`='$contactnum',`address`='$address',date_baptize='$bap',`contactstatus_id`='$contstat',`shepmaterial`='$shep',gender='$gender'
 WHERE baptism_id='$contacid'")or die(mysql_error());

echo '<script> swal({title: "Praise the Lord!",text: "Data already submitted!", customClass: "swal-wide",
confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("addbaptism.php?id='.$get_id.'","_self")});</script>';
exit();
