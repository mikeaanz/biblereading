...
$sql=mysql_select_db('dbmonitoring',mysql_connect('localhost','root',''))or die(mysql_error());
  ...
  $rowz = mysql_fetch_array($qwery);
     ...
     $this->Cell(270,10,$rowz['PLACES'],0,0,'C');

     $this->Cell(-274);
          $this->Cell(279,20,'MONITORED CONTACTS',0,0,'C');
      $this->Ln();
...
$this->SetFont('Times','',9);
  $this->Setwidths(Array(44,37,37,10,30,30,30,30,30,15,15,15,19,19,15,17,15,15));
 $this->setAligns(Array('C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C','C'));
 $this->SetLineHeight(5);

 $this->Row(Array('','','','','Week 1','Week 2','Week 3','Week 4','Week 5'));
 $this->Row(Array('Name','Address','Contact No(s)','S-LD','M-T-W-Th-F-S-LD','M-T-W-Th-F-S-LD','M-T-W-Th-F-S-LD','M-T-W-Th-F-S-LD','M-T-W-Th-F-S-LD'));
...
SELECT * from longproprpt
  LEFT join contatcdetails on contatcdetails.contact_id=longproprpt.contact_id
   LEFT join historyfeedback on historyfeedback.id=longproprpt.historyfeedback_id
...
where   longproprpt.acc_id='".$_GET['SessionId']."' and  batch.BATCH='".$_GET['TermNumber']."'
ORDER BY `longproprpt`.`longproprpt_id` ASC
...
")or die(mysql_error());
                     ...
                     $this->Row(Array($row['FullName'],$row['address'],$row['ContactNumber'],$row['Sun_LD'],$row['Week_ONE'],$row['Week_TWO'],$row['Week_THREE'],$row['Week_FOUR'],$row['Week_FIVE']));            
...
$pdf->Cell(45);
$pdf->SetFont('Times','',8);
$pdf->Cell(70,30,'Week Activites',0,0,'C');

$pdf->Ln(15);
$pdf->Cell(70);
$pdf->MultiCell(275,7,'CO-Contact, BP-Baptized, V-Visit, O-Out/Unavailable, F-Fellowship, P-Prayer, S-Sharing, MR-Morning Revival, BR-Bible Reading',0,'L',false);
$pdf->Ln(-4);
$pdf->Cell(70);
$pdf->MultiCell(275,7,'HS-Hymn Singing, LTM-Lord table Meetings, SG-Small Group Mtg, Pm-Prayer Mtg, NB#-New Believer Series, LL#-Life Lesson, IS$-Intensified Shepherding ',0,'L',false);

$pdf->Ln(-4);
$pdf->Cell(70);
$pdf->MultiCell(275,7,'HG#-High Gospel, AB#-After Being Saved, TO#-Trust and Obey, SL#-Spirit and Life, KT#-Knowing the Truth',0,'L',false);
