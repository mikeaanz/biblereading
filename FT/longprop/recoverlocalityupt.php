...
<div class="col-xl-12">
  <div class="card shadow mb-4">
    ...
    <?php
							$user_query = mysql_query("SELECT * FROM recoverlocal 
						 where recoverlocal.recover_id='$ids'
							")or die(mysql_error());
     ...
     ?>
            ...
            <label class="control-label" >Name of Recovered Locality:</label>
                        <input class="form-control form-control-lg" name="name" value="<?php echo $row['name'];?>" id="price" type="text"  />

                        <div class="dates">
			<label class="dates" for="price">Date Recovered:</label>
			<input class="form-control form-control-lg" data-toggle="datepicker" readonly onkeypress="return isNumberKey(event)" style="background-color:#aed6f1;" value="<?php echo $row['daterecover'];?>"  id="usr1" name="date"  placeholder="mm-dd-yyyy" autocomplete="off" required/>
...
$daterec=$_POST['date'];
  ...
  $act = "SELECT * FROM recoverlocal WHERE recover_id='$ids'";
...
if($activated > 0){
    ...
    $result =mysql_query("UPDATE `recoverlocal` SET
   `name`='$name',
     `daterecover`='$daterec'
    where recover_id='$ids'")or die(mysql_error());
    
	echo '<script> swal({title: "Praise the Lord",text: "Your Data Successfully Updated!", customClass: "swal-wide",
        confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("editrecoverlocal.php?locate_idpost='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
      exit();
...
echo '<script> swal({title: "Notice!",text: "Your data already existing.", customClass: "swal-wide",
    confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("editrecoverlocal.php?locate_idpost='.$back.'&id='.$ids.'&BATCH='.$getbatch.'","_self")});</script>';
  exit();
