...
<div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <input type="password" class="form-control" name="secure"  placeholder="Security Code" required>
  </div>
...
if (isset($_POST['delete_user'])){
     ...
     echo '<script> swal({title: "Notice!",text: "Please Check the checkbox before to proceed!", customClass: "swal-wide",
      confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("editreportinfo.php","_self")});</script>';
    exit();
  }
       if($activated > 0){
          $id=$_POST['selector'];
            ...
            for($i=0; $i < $N; $i++)
            {
                ...
                echo '<script> swal({title: "Security Code Match!",text: "This Data is Already Deleted!", customClass: "swal-wide",
                    confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("editreportinfo.php","_self")});</script>';
                    exit();
}
