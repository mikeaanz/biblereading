...
<div class="col-xl-3">
  <div class="card shadow mb-4">
      <h6 class="m-3 font-weight-bold text-primary">Prospect Trainee Information</h6>
    <!-- Card Body -->
    <form  method="POST">
    <div class="card-body">
    ...
    <?php
							...
							INNER JOIN stat_saints on stat_saints.stat_id =  prospect_train.Status where prospect_train.pros_id='$ids'
     ...
     ?>
			...
			<input class="form-control form-control-lg" name="cell" id="price" type="Number" value=<?php echo $row['cellphone'];?> placeholder="Example +6393045311711" maxlength="11" />
...
$cell=$_POST['cell'];
  ...
  $act = "SELECT * FROM prospect_train WHERE pros_id='$ids'";
...
if($activated > 0){
	...
	echo '<script> swal({title: "Good Job!",text: "Your Data Successfully Updated!", customClass: "swal-wide",
        confirmButtonColor: "#0BA9F9",type: "success"},function(){window.open("prospectedit.php?historyfeedbackid='.$back.'&id='.$ids.'","_self")});</script>';
      exit();
...
echo '<script> swal({title: "Notice!",text: "Your data already existing.", customClass: "swal-wide",
    confirmButtonColor: "#0BA9F9",type: "error"},function(){window.open("prospectedit.php?historyfeedbackid='.$back.'&id='.$ids.'","_self")});</script>';
  exit();
