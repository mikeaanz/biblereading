...
<!--             <h1 class="h3 mb-0 text-gray-800">Dashboard</h1> -->
  ...
  $rowz = mysql_fetch_array($qwery);
            ...
            $termquery = mysql_query("SELECT * FROM batch,year,month,userlevel
                        where batch.BATCH='$term' and LEVEL='WEEKEND'
 ...
 <a id="btnGenerateReport" href="#"  onclick="window.open('weekendpdf.php?SessionId=<?php echo  $locality; ?> &TermNumber= <?php echo $term ?>', 'newwindow', 'width=900, height=600'); return false;" class="d-show d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Generate Report</a> 
         ...
         <ul class="breadcrumb">
          ...
          <?php 
                                ...
                                where batch.BATCH='$term'  and userlevel.LEVEL='WEEKEND' or userlevel.LEVEL='5 DAYS PROPAGATION'
          ...
          <?php }?>
                       ...
                       <div class="h5 mb-0 font-weight-bold text-gray-800 counter" data-count="<?php echo $row['LOCALATTSMLMTG'];?>">0</div>
                        ...
                        <?php 
                                $query = mysql_query("select SUM(LTM) as ltm  from weekspropagation
                                inner join historyfeedback on historyfeedback.id=weekspropagation.historyfeedback_id
                                inner join month on month.id=historyfeedback.MONTH
                                inner join year on year.id=historyfeedback.YEAR 
                                inner join batch on batch.id=historyfeedback.BATCH 
