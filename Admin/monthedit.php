...
<?php
   					$user_query = mysql_query("SELECT * from month where ID='$get_id'")or die(mysql_error());
...
?>
    ...
    <form  method="POST">
        ...
        <div class="col mr-2">
          <div class="text-xs font-weight-bold text-info text-uppercase mb-1"><i ></i>Month</div>
...
<input class="form-control form-control-sm" name="year" id="cost" type="text"   onkeypress="return isNumberKey(event)" value="<?php echo $row['MONTH']; ?>"  placeholder="Please Enter Year" required/>
...
if($activated > 0){
    ...
    echo '<script> swal({title: "OH NO!",text: "This Month can not be updated due of already exists!", customClass: "swal-wide",
    ...
    mysql_query("UPDATE `month` SET `MONTH`='$month' WHERE ID='$get_id'")or die(mysql_error());
